...
private:
    ...
    void rotateFace(int ind) {
        uint64_t temp=bitboard[ind]>>(8*6);
        uint64_t mask=bitboard[ind];
        bitboard[ind]=mask<<16;
        bitboard[ind]=bitboard[ind]|(temp);
    }
    void rotateSide(int s1,int s1_1,int s1_2,int s1_3,int s2,int s2_1,int s2_2,int s2_3) {
        uint64_t clr1 = (bitboard[s2]&(one_8<<(s2_1*8)))>>(s2_1*8);
        uint64_t clr2 = (bitboard[s2]&(one_8<<(s2_2*8)))>>(s2_2*8);
        uint64_t clr3 = (bitboard[s2]&(one_8<<(s2_3*8)))>>(s2_3*8);

        bitboard[s1]=(bitboard[s1]&(~(one_8<<(s1_1*8))))|(clr1<<(s1_1*8));
        bitboard[s1]=(bitboard[s1]&(~(one_8<<(s1_2*8))))|(clr2<<(s1_2*8));
        bitboard[s1]=(bitboard[s1]&(~(one_8<<(s1_3*8))))|(clr3<<(s1_3*8));

    }

    //Helper to get 5bit corner
    int get5bitCorner(string corner) {
        ...
        if(actual_string[0]==corner[1]) {
            ret|=(1<<3);
        }
        if(actual_string[0]==corner[2]) {
            ret|=(1<<4);
        }
        return ret;
...
public:
    ...
    RubiksCubeBitboard() {
        for(unsigned int side=0;side<6;side++) {
            uint64_t mask=0;
            uint64_t curr=(1<<side);
            for(unsigned int faceIdx=0;faceIdx<8;faceIdx++) {
                mask=mask|(curr<<(8*faceIdx));
            }
            bitboard[side]=mask;
            solved_side_config[side]=mask;
        }
    }
    ...
    COLOR getColor(FACE face, unsigned row, unsigned col) const override {
        ...
        uint64_t mask=(side>>(8*idx))&one_8;
    ...
    bool isSolved() const override {
        for(int face=0;face<6;face++) {
            if(solved_side_config[face]!=bitboard[face])return false;
        }
        return true;
    }
    ...
    bool operator==(const RubiksCubeBitboard &r1) const {
        for (int i = 0; i < 6; i++) {
            if (r1.bitboard[i] != bitboard[i]) return false;
        }
        return true;
    }
    ...
    uint64_t getCorners() {
        ...
        bottom_front_right+=getColorLetter(getColor(FACE::LEFT,2,0));
        ...
        ret=ret|get5bitCorner(top_front_right);
        ret=(ret<<5)|get5bitCorner(top_front_left);
        ret=(ret<<5)|get5bitCorner(top_back_right);
        ret=(ret<<5)|get5bitCorner(top_back_left);
        ret=(ret<<5)|get5bitCorner(bottom_front_right);
        ret=(ret<<5)|get5bitCorner(bottom_front_left);
        ret=(ret<<5)|get5bitCorner(bottom_back_right);
        ret=(ret<<5)|get5bitCorner(bottom_back_left);
        ret=(ret<<5);
...
struct HashBitboard {
    size_t operator()(const RubiksCubeBitboard &r1) const {
        uint64_t final_bitboard=r1.bitboard[0];
        for(int i=1;i<6;i++) {
            final_bitboard=final_bitboard^(r1.bitboard[i]);
        }
        return (size_t)final_bitboard;
